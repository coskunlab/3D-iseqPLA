...
# (Pearson colocalization of p65 with DAPI signal per cell), and
...
plot_output_dir = FIGURES_ROOT / '94_plot_p65_activation_coloc_plate021'
...
def calculate_p65_activation_per_cell(df):
    """
    Calculate p65 activation as Pearson colocalization between p65 and DAPI signals per cell.

    Parameters:
    -----------
    df : pandas.DataFrame
        Single-cell data with columns: CellLabel, p65, and DAPI/Hoechst/HOECHST
    ...
    """
    # Calculate Pearson correlation between p65 and DAPI for each cell
    activation_data = []
    
    # Auto-detect the DAPI column name
    dapi_column = None
    # First try exact matches
    possible_dapi_names = ['Cycle1 DAPI', 'DAPI', 'dapi', 'Hoechst', 'HOECHST', 'hoechst', 'Hoescht', 'HOESCHT']
    for col_name in possible_dapi_names:
        if col_name in df.columns:
            dapi_column = col_name
            break
    
    # If not found, look for any column containing 'DAPI' or 'dapi'
    if dapi_column is None:
        for col_name in df.columns:
            if 'DAPI' in col_name or 'dapi' in col_name:
                dapi_column = col_name
                break
    
    if dapi_column is None:
        # Print available columns for debugging
        print(f"Warning: No DAPI column found. Available columns: {df.columns.tolist()}")
        return pd.DataFrame()

    for cell_id, cell_df in df.groupby('CellLabel'):
        # Get p65 and DAPI signals for the entire cell
        p65_signal = cell_df['p65'].values
        dapi_signal = cell_df[dapi_column].values
        
        # Only calculate if we have sufficient data points
        if len(p65_signal) > 2 and len(dapi_signal) > 2:
            if np.std(p65_signal) > 0 and np.std(dapi_signal) > 0:
                # Calculate Pearson correlation
                pearson_r = np.corrcoef(p65_signal, dapi_signal)[0, 1]
                
                if not np.isnan(pearson_r):
                    activation_data.append({
                        'CellLabel': cell_id,
                        'p65_activation': pearson_r,
                        'p65_mean': p65_signal.mean(),
                        'dapi_mean': dapi_signal.mean()
                    })
...
if len(valid_results) == 0:
    ...
    print("This likely means the DAPI column was not found.")
...
if not combined_df.empty:
    ...
    ax.set_ylabel("p65-DAPI Colocalization\\n(Pearson r)")
    ...
    for aggregation_method in ['mean', 'median']:
        ...
        if not pivot_data.empty:
            ...
            sns.heatmap(
                ...
                annot=False,
                cmap='coolwarm',
                cbar_kws={'label': f'{aggregation_method.capitalize()} p65 Activation'},
                ax=ax
            )

            ax.set_title(f"{aggregation_method.capitalize()} p65 Activation Heatmap - Plate 021", pad=20)
            ax.set_xlabel("Timepoint (minutes)")
            ax.set_ylabel("Condition")

            plt.tight_layout()
            plot_filename = plot_output_dir / f"heatmap_p65_activation_{aggregation_method}.png"
            plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
            print(f"Saved: {plot_filename.name}")
            plt.close(fig)
    ...
    # %% [markdown]
    # ### Plot 4b: Heatmap - Cell Count by Condition and Time

    # %%
    pivot_counts = combined_df.pivot_table(
        values='CellLabel',
        index='Condition',
        columns='Timepoint',
        aggfunc='nunique'
    ...
    if not pivot_counts.empty:
        ...
        ax.set_title("Cell Count Heatmap - Plate 021", pad=20)
        ax.set_xlabel("Timepoint (minutes)")
        ax.set_ylabel("Condition")

        plt.tight_layout()
        plot_filename = plot_output_dir / "heatmap_cell_count.png"
        plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
        print(f"Saved: {plot_filename.name}")
        plt.close(fig)
    ...
    if not df_45min.empty:
        ...
        # %%
        # Define activated cells: p65_activation > 0.5 (positive colocalization)
        # Define unactivated cells: p65_activation <= 0.5 (low/negative colocalization)
        df_45min['activation_status'] = df_45min['p65_activation'].apply(
            lambda x: 'Activated' if x > 0.5 else 'Unactivated'
        )
