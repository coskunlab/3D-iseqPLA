...
N_TOP_GENES = 5  # Number of top important genes to highlight
...
# ============================================================================
# Load Gene Vocabulary from Training
# ============================================================================
print("="*100)
print("Loading Gene Vocabulary from Training")
print("="*100)
...
# Create full vocab with special tokens (matching training configuration)
vocab = gene_vocab.copy()
vocab[PAD_TOKEN] = len(gene_vocab)
vocab[CLS_TOKEN] = len(gene_vocab) + 1
VOCAB_SIZE = len(vocab)

print(f"[OK] Vocabulary size (with special tokens): {VOCAB_SIZE:,}")

# Also load the gene names list from vocab for later use
gene_names = sorted(gene_vocab.keys())

# ============================================================================
# Load Expression Data for Gene Importance Analysis
# ============================================================================
print(chr(92) + 'n' + '='*100)
...
if not data_file.exists():
    ...
    # Update gene_names to match the filtered genes in adata
    gene_names = adata.var_names.tolist()
    print(f'  Cells: {adata.n_obs:,}')
    print(f'  Genes: {adata.n_vars:,}')
    print('[OK] Expression data loaded')
    print(f'[OK] Updated gene_names to match filtered genes: {len(gene_names):,} genes')
...
model = TransformerModel(
    ...
    d_hid=D_HID,
    nlayers=N_LAYERS,
    nlayers_cls=2,  # Classification layers (must match training)
    n_cls=2,  # Binary classification (must match training)
    vocab=vocab,
    dropout=DROPOUT,
    pad_token=PAD_TOKEN,
    pad_value=PAD_VALUE,
...
for idx, (metric, label) in enumerate(zip(metrics_to_compare, metric_labels)):
    ...
    # Always include all spaces, even if NaN
    for space in spaces_to_plot:
        ...
        if space in distance_summary and metric in distance_summary[space]:
            ...
            values.append(val if not np.isnan(val) else 0)  # Use 0 for NaN
        else:
            values.append(0)

    if values:
        # Create bar colors (gray for NaN/missing values)
        bar_colors = []
        for i, space in enumerate(spaces_to_plot):
            if space in distance_summary and metric in distance_summary[space]:
                val = distance_summary[space][metric]
                if np.isnan(val):
                    bar_colors.append('#CCCCCC')  # Gray for NaN
                else:
                    bar_colors.append('steelblue')
            else:
                bar_colors.append('#CCCCCC')  # Gray for missing
        
        bars = ax.bar(range(len(values)), values, color=bar_colors, alpha=0.7)
        ...
        # Highlight best value (lowest for distances, highest for similarity) - only among valid values
        valid_values = [(i, v) for i, v in enumerate(values) if v > 0]
        if valid_values:
...
# Always include all spaces
for space in spaces_to_plot:
    space_names.append(space)
    if space in distance_summary and 'knn_overlap_mean' in distance_summary[space]:
        ...
        overlap_means.append(0)
        overlap_stds.append(0)

x_pos = np.arange(len(space_names))

# Color bars based on data validity
bar_colors = []
for i, space in enumerate(spaces_to_plot):
    if space in distance_summary and overlap_means[i] > 0:
...
# Highlight best value (only among valid values)
valid_means = [(i, m) for i, m in enumerate(overlap_means) if m > 0]
if valid_means:
    best_idx = max(valid_means, key=lambda x: x[1])[0]
    bars[best_idx].set_color('orange')
...
# Set y-limit based on valid values
valid_mean_values = [m for m in overlap_means if m > 0]
if valid_mean_values:
    ax.set_ylim(0, max(valid_mean_values) * 1.2)
else:
    ax.set_ylim(0, 10)
...
for i, (mean, std) in enumerate(zip(overlap_means, overlap_stds)):
    if mean > 0:
        ...
        ax.text(i, 1, 'N/A', ha='center', va='bottom', fontweight='bold',
...
for space in spaces_to_plot:
    ...
    if space in distance_summary and 'silhouette_score' in distance_summary[space]:
        ...
        silhouette_scores.append(score if not np.isnan(score) else 0)
    else:
        silhouette_scores.append(0)

x_pos = np.arange(len(space_names))

# Color based on validity and value
colors = []
for i, space in enumerate(spaces_to_plot):
    if space in distance_summary and 'silhouette_score' in distance_summary[space]:
        score = distance_summary[space]['silhouette_score']
...
# Highlight best value (only among valid values)
valid_scores = [(i, s) for i, s in enumerate(silhouette_scores) if s != 0]
if valid_scores:
...
# Set y-limits based on valid scores
valid_score_values = [s for s in silhouette_scores if s != 0]
if valid_score_values:
    ax.set_ylim(min(valid_score_values) * 1.2 if min(valid_score_values) < 0 else -0.1,
               max(valid_score_values) * 1.2)
else:
    ax.set_ylim(-0.1, 0.1)

# Add value labels
for i, score in enumerate(silhouette_scores):
    space = spaces_to_plot[i]
    if space in distance_summary and 'silhouette_score' in distance_summary[space]:
        actual_score = distance_summary[space]['silhouette_score']
        if np.isnan(actual_score):
            ax.text(i, 0, 'N/A', ha='center', va='center', fontweight='bold', 
                   style='italic', color='gray')
        else:
...
# First pass: collect all p-values from both spaces to determine common x-axis limit
all_valid_p_values = []
for space in focus_spaces:
    for test_key, test_name in stat_tests:
        p_val = distance_summary[space].get(test_key, np.nan)
        if not np.isnan(p_val):
            all_valid_p_values.append(p_val)

# Determine common x-axis limit for both subplots
common_max_p_val = max(all_valid_p_values) if all_valid_p_values else 1.0
common_xlim = common_max_p_val * 1.3
...
for idx, space in enumerate(focus_spaces):
    ...
    # Always include all tests, even if NaN
    for test_key, test_name in stat_tests:
        p_val = distance_summary[space].get(test_key, np.nan)
        p_values.append(p_val)
        test_names.append(test_name)

        # Color based on significance (gray if NaN)
        if np.isnan(p_val):
    ...
    # Replace NaN with 0 for plotting (will show as empty bars)
    p_values_plot = [0 if np.isnan(v) else v for v in p_values]
    
    bars = ax.barh(test_names, p_values_plot, color=colors_list, alpha=0.8,
                   edgecolor='black', linewidth=1.5)
...
def format_val(val, fmt='.4f'):
    """Format value, return 'N/A' if NaN"""
    if np.isnan(val):
        return 'N/A'
    return f"{val:{fmt}}"

def compare_vals(val_512d, val_pca, higher_is_better=False):
    """Compare two values and return which is better"""
    if np.isnan(val_512d) and np.isnan(val_pca):
        return 'N/A'
    elif np.isnan(val_512d):
        return 'PCA'
    elif np.isnan(val_pca):
        return '512D'
    else:
        if higher_is_better:
            return 'PCA' if val_pca > val_512d else '512D'
        else:
            return 'PCA' if val_pca < val_512d else '512D'

# Create summary data with safe value access
summary_data = [
    ['Metric', 'Original 512D', 'PCA', 'Better'],
    ['', '', '', ''],
    ['DISTANCE METRICS', '', '', ''],
    ['Euclidean Mean', 
     format_val(distance_summary['Original_512D'].get('euclidean_mean', np.nan)),
     format_val(distance_summary['PCA'].get('euclidean_mean', np.nan)),
     compare_vals(distance_summary['Original_512D'].get('euclidean_mean', np.nan),
                  distance_summary['PCA'].get('euclidean_mean', np.nan))],
    ['Euclidean Median',
     format_val(distance_summary['Original_512D'].get('euclidean_median', np.nan)),
     format_val(distance_summary['PCA'].get('euclidean_median', np.nan)),
     compare_vals(distance_summary['Original_512D'].get('euclidean_median', np.nan),
                  distance_summary['PCA'].get('euclidean_median', np.nan))],
    ['Cosine Similarity',
     format_val(distance_summary['Original_512D'].get('cosine_sim_mean', np.nan)),
     format_val(distance_summary['PCA'].get('cosine_sim_mean', np.nan)),
     compare_vals(distance_summary['Original_512D'].get('cosine_sim_mean', np.nan),
                  distance_summary['PCA'].get('cosine_sim_mean', np.nan), higher_is_better=True)],
    ['Manhattan Mean',
     format_val(distance_summary['Original_512D'].get('manhattan_mean', np.nan)),
     format_val(distance_summary['PCA'].get('manhattan_mean', np.nan)),
     compare_vals(distance_summary['Original_512D'].get('manhattan_mean', np.nan),
                  distance_summary['PCA'].get('manhattan_mean', np.nan))],
    ['Mahalanobis Mean',
     format_val(distance_summary['Original_512D'].get('mahalanobis_mean', np.nan)),
     format_val(distance_summary['PCA'].get('mahalanobis_mean', np.nan)),
     compare_vals(distance_summary['Original_512D'].get('mahalanobis_mean', np.nan),
                  distance_summary['PCA'].get('mahalanobis_mean', np.nan))],
    ['Hausdorff Distance',
     format_val(distance_summary['Original_512D'].get('hausdorff_symmetric', np.nan)),
     format_val(distance_summary['PCA'].get('hausdorff_symmetric', np.nan)),
     compare_vals(distance_summary['Original_512D'].get('hausdorff_symmetric', np.nan),
                  distance_summary['PCA'].get('hausdorff_symmetric', np.nan))],
    ['', '', '', ''],
    ['CLUSTER METRICS', '', '', ''],
    ['Silhouette Score',
     format_val(distance_summary['Original_512D'].get('silhouette_score', np.nan)),
     format_val(distance_summary['PCA'].get('silhouette_score', np.nan)),
     compare_vals(distance_summary['Original_512D'].get('silhouette_score', np.nan),
                  distance_summary['PCA'].get('silhouette_score', np.nan), higher_is_better=True)],
    ['KNN Overlap (k=10)',
     format_val(distance_summary['Original_512D'].get('knn_overlap_mean', np.nan), '.2%'),
     format_val(distance_summary['PCA'].get('knn_overlap_mean', np.nan), '.2%'),
     compare_vals(distance_summary['Original_512D'].get('knn_overlap_mean', np.nan),
                  distance_summary['PCA'].get('knn_overlap_mean', np.nan), higher_is_better=True)],
    ['', '', '', ''],
    ['STATISTICAL TESTS', '', '', ''],
    ['Mann-Whitney p-value',
     format_val(distance_summary['Original_512D'].get('mannwhitney_p', np.nan)),
     format_val(distance_summary['PCA'].get('mannwhitney_p', np.nan)),
     compare_vals(distance_summary['Original_512D'].get('mannwhitney_p', np.nan),
                  distance_summary['PCA'].get('mannwhitney_p', np.nan))],
    ['K-S Test p-value',
     format_val(distance_summary['Original_512D'].get('ks_p', np.nan)),
     format_val(distance_summary['PCA'].get('ks_p', np.nan)),
     compare_vals(distance_summary['Original_512D'].get('ks_p', np.nan),
                  distance_summary['PCA'].get('ks_p', np.nan))],
    ['Permutation p-value',
     format_val(distance_summary['Original_512D'].get('permutation_p', np.nan)),
     format_val(distance_summary['PCA'].get('permutation_p', np.nan)),
     compare_vals(distance_summary['Original_512D'].get('permutation_p', np.nan),
                  distance_summary['PCA'].get('permutation_p', np.nan))],
]
...
mw_512d = distance_summary['Original_512D'].get('mannwhitney_p', np.nan)
mw_pca = distance_summary['PCA'].get('mannwhitney_p', np.nan)

spaces = ['Original 512D', 'PCA']
p_values = [mw_512d, mw_pca]

# Color based on significance (gray for NaN)
colors = []
...
bars = ax.barh(spaces, p_values_plot, color=colors, alpha=0.8,
               edgecolor='black', linewidth=2, height=0.5)

# Determine x-axis limit from valid values
valid_p_values = [v for v in p_values if not np.isnan(v)]
max_p_val = max(valid_p_values) if valid_p_values else 1.0
...
for i, (space, p_val) in enumerate(zip(spaces, p_values)):
    if np.isnan(p_val):
        ax.text(max_p_val * 0.5, i, '  N/A (insufficient data)',
...
# Determine overall interpretation based on valid values
valid_significant = [v for v in valid_p_values if v < 0.05]
if len(valid_significant) == len(valid_p_values) and len(valid_p_values) > 0:
    interpretation = "Both spaces show p < 0.05: Select genes ARE significantly closer to important genes!"
elif len(valid_significant) > 0:
    interpretation = f"{len(valid_significant)} of {len(valid_p_values)} spaces show significant proximity (p < 0.05)"
else:
    interpretation = "Check individual results for significance"
